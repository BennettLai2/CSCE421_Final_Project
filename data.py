...
def preprocess_x(df):
    ...
    condensed_df1 = df.pivot_table(
        index=['patientunitstayid', 'offset', 'admissionheight', 'admissionweight', 'age', 'ethnicity', 'gender',
               'unitvisitnumber'],
    ...
    condensed_df4 = pd.merge(condensed_df1, condensed_df2,
                             on=['patientunitstayid', 'unitvisitnumber', 'offset', 'admissionheight', 'admissionweight',
                                 'age', 'ethnicity', 'gender'], how='outer')
    condensed_df = pd.merge(condensed_df3, condensed_df4,
                            on=['patientunitstayid', 'unitvisitnumber', 'offset', 'admissionheight', 'admissionweight',
                                'age', 'ethnicity', 'gender'], how='outer')

    df = condensed_df.sort_values(by=['patientunitstayid', 'offset'])
    df.columns = ["patientunitstayid", "offset", "admissionheight", "admissionweight", "age", "ethnicity", "gender",
                  "unitvisitnumber", "Capillary Refill", "GCS Total", "Heart Rate", "Invasive BP Diastolic",
                  "Invasive BP Mean", "Invasive BP Systolic", "Non-Invasive BP Diastolic", "Non-Invasive BP Mean",
                  "Non-Invasive BP Systolic", "O2 Saturation", "Respiratory Rate", "glucose", "pH"]

    df['Capillary Refill'] = df['Capillary Refill'].replace(-1, pd.NaT)
    combined_df = df[['Invasive BP Diastolic', 'Invasive BP Mean', 'Invasive BP Systolic', 'Non-Invasive BP Diastolic',
                      'Non-Invasive BP Mean', 'Non-Invasive BP Systolic']].copy()
    combined_df['BP Diastolic'] = combined_df['Invasive BP Diastolic'].combine_first(
        combined_df['Non-Invasive BP Diastolic'])
    combined_df['BP Mean'] = combined_df['Invasive BP Mean'].combine_first(combined_df['Non-Invasive BP Mean'])
    combined_df['BP Systolic'] = combined_df['Invasive BP Systolic'].combine_first(
        combined_df['Non-Invasive BP Systolic'])
    ...
    df['BP Mean'] = df['BP Mean'].fillna(
        df['BP Diastolic'].shift(1) + (df['BP Systolic'].shift(-1) - df['BP Diastolic'].shift(1)) / 3)
    ...
    cols = ['Capillary Refill', 'GCS Total', 'Heart Rate', 'O2 Saturation', 'Respiratory Rate', 'glucose', 'pH',
            'BP Diastolic', 'BP Mean', 'BP Systolic']
